...
import { Scan, ShieldCheck, ShieldX, Loader2, History } from "lucide-react";
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="p-8 rounded-3xl bg-card border border-border shadow-xl backdrop-blur-xl space-y-6">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    autoFocus
                    className="flex-1 px-4 py-3 bg-background border border-input rounded-xl text-sm font-mono text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500"
                  />
                  <button
                    type="submit"
          ...
          {lastResult && (
            <div
              className={`p-8 rounded-3xl border shadow-2xl transition-all duration-300 ${lastResult.status === "APPROVED"
                ? "bg-emerald-500/10 dark:bg-emerald-950/40 border-emerald-500/60 shadow-emerald-500/10"
                : "bg-rose-500/10 dark:bg-rose-950/40 border-rose-500/60 shadow-rose-500/10"
                }`}
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center ${lastResult.status === "APPROVED"
                  ...
                  <div className="space-y-1">
                    <span
                      className={`text-xs font-black uppercase tracking-widest px-3 py-1 rounded-full ${lastResult.status === "APPROVED"
                    ...
                    <h3 className="text-xl font-bold text-foreground pt-1">{lastResult.message}</h3>
                    {lastResult.rejectionReason && (
                      <p className="text-xs text-rose-600 dark:text-rose-300 font-medium">Reason: {lastResult.rejectionReason}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {lastResult.pass && (
                <div className="mt-6 pt-6 border-t border-border/50 grid grid-cols-2 gap-4 text-xs">
                  <div>
                  ...
                  <div className="col-span-2">
                    ...
                    <p className="font-mono text-xs text-purple-600 dark:text-purple-300 font-semibold">{lastResult.pass.token}</p>
                  </div>
                </div>
              )}
            </div>
